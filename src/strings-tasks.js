...
function removeFirstOccurrences(str, value) {
  if (str.includes(value)) {
    const res1 = str.substring(0, str.indexOf(value));
    const res2 = str.substring(str.indexOf(value) + value.length, str.length);
    return res1 + res2;
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  if (str.includes(value)) {
    const res1 = str.substring(0, str.lastIndexOf(value));
    const res2 = str.slice(str.lastIndexOf(value) + value.length);
    return res1 + res2;
  }
  return str;
}
...
function isPalindrome(str) {
  const lowerStr = str.toLowerCase().replace(/[^a-zA-Z]/g, '');
  return Array.from(lowerStr).reverse().join('') === lowerStr;
}
...
function reverseWords(str) {
  ...
  const reverceWords = words.map((item) => Array.from(item).reverse().join(''));
  return reverceWords.join(' ');
}
...
function invertCase(str) {
  const letters = Array.from(str).map((item) =>
...
function extractNameFromTemplate(value) {
  const params = value.match(/Hello, (\w+) (\w+)!/);
...
function encodeToRot13(str) {
  ...
  const rot13 = Array.from(str).map((item) =>
    alphas.includes(item)
      ? alphas[(alphas.indexOf(item) + 13) % alphas.length]
      : item
  );
  return rot13.join('');
}
